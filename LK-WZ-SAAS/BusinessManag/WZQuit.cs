...
{
    public partial class WZQuit : Form, IPlug
    {
        private form.QuitEdit editForm = null;
        ...
        {
            InitializeComponent();
            TvList.newBind().add("作废", "0").add("等待审核", "1").add("已审核", "2").add( "已退回","6").Bind(this.comboBox2);
...
//已调拨
        ...
        #endregion
        private void WZQuit_Load(object sender, EventArgs e)
        {
           // TvList.newBind().add("》").Bind(this.Column24);
            ...
            this.dataGView1.Url = "QuitFindOutMainList";
        ...
        public void reLoad()  //根据查询条件，重新加载结果
        {
            ...
            if (this.dataGView1.RowCount > 0)
            {
                for (int i = 0; i < this.dataGView1.RowCount; i++)
                {
                    ...
                    {
                        string string1 = LData.Es("QuitMixiBishu", "LKWZ", new object[] { this.dataGView1["Column1", i].Value, His.his.Choscode });
        ...
        private void toolStripButton6_Click(object sender, EventArgs e)   //新增
        {
            ...
            this.editForm = new form.QuitEdit(null,2,app);
            this.editForm.ShowDialog();
           // //this.dataGView1.DataSource = null;
           // //this.dataGView1.BindDataTable = null;
           // //this.dataGView2.DataSource = null;
           // //this.dataGView2.BindDataTable = null;
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)  //编辑
        {
            ...
            this.editForm = new form.QuitEdit(info, status, app);
            this.editForm.ShowDialog();
        ...
        void reBindMdWare(object CKWareId)
        {
            ...
            //r = WzWare.Select("IFSTWARE=" + r[0]["IFSTWARE"].ToString() + "and IFSTWARE=" + r[0]["IFSTWARE"].ToString() + "and IFSTWARE=" + r[0]["IFSTWARE"].ToString() + "and WARECODE <> '" + r[0]["WARECODE"].ToString() + "'");
            //TvList tv = TvList.newBind();
            //((DataGridViewComboBoxCell)this.dataGView1.Rows[xBRowIndex].Cells["Column5"]).Items.Clear();
            //for (int i = 0; i < r.Length; i++)
            //{
        ...
        void Sh_ServiceLoad(object sender, YtClient.data.events.LoadEvent e) //物资库房绑定
        {
                ...
                {
                    if (e.Msg.GetDataTable().Rows[0]["SYSVALUE"].ToString() == "0")//需确认退回
                    {
                        this.dataGView1.GetRowData()["STATUS"] = "2";
        ...
        private void toolStripButton2_Click(object sender, EventArgs e) //调拨退回
        {
            //if (this.dataGView1.SelectedRows == null || this.dataGView1.SelectedRows.Count < 1)
            if (this.dataGView1.CurrentRow == null)
            {
            ...
            if (this.dataGView1.GetRowData()["STATUS"].ToString() != "2")
            {
                WJs.alert("只能对处于已审核状态的数据进行退回确认，请选择已审核的行！");
                return;
            }
        ...
        private void scan_toolStripButton_Click(object sender, EventArgs e)
        {
            ...
            info.Add("outID", this.dataGView1.CurrentRow.Cells["Column1"].Value.ToString());
            info.Add("outDate", this.dataGView1.CurrentRow.Cells["Column8"].Value.ToString());
            this.editForm = new form.QuitEdit(info, 0, app);
            this.editForm.ShowDialog();
        }
