...
{
    public partial class ShenLingEdit : Form
    {
        ...
        string preText = null;
        string preValue1 = null;
        string preText1 = null;

        YtWinContrl.com.contrl.SelTextInpt Wz =null;
        public ShenLingEdit(Dictionary<string, string> info ,int isEdit,IAppContent app)
        {
            this.app = app;
            this.isEdit = isEdit;
            outInfo = info;
            InitializeComponent();
            this.selTextInpt1.SelParam = His.his.Choscode + "|0|0|2|2" + "|{key}|{key}|{key}|{key}|{key}|{key}";  //0表示选择不选择本级库房，1表示选择本级库房，2表示对本级库房不做限制
            this.selTextInpt2.SelParam = His.his.Choscode + "|0|2|2|0" + "|{key}|{key}|{key}|{key}|{key}|{key}";
            this.dataGView1.EntHh = false;
            //this.selTextInpt3.SelParam = "1|1|1|" + His.his.Choscode;
        }
        private void reNum()
        {
            int num = 1;
            for (int i = 0; i < this.dataGView1.Rows.Count; i++)
            {
                ...
                {
                    this.dataGView1.Rows[i].Cells["LiuShuiNum"].Value = num;
                    num++;
                }
            }
        }
        private void toolStripButton3_Click(object sender, EventArgs e) //新增物质
        {
            ...
            if (this.selTextInpt2.Value == null || this.selTextInpt2.Value == "")
            {
                WJs.alert("请选择申领目的库房！");
                return;
            }
            //改变查询条件 这里是因为查询Opt中需要a.cHosCode=? and 仓库编号 = ? 参数进行查询
            this.dataGView1.GetSql("物资").Ps = His.his.Choscode + "|" + this.selTextInpt2.Value + "|" + isAllowFKC.ToString() + "|{key}|{key}|{key}|{key}|{key}|{key}";
        ...
        private void toolStripButton2_Click(object sender, EventArgs e) //删除物质
        {
            if (this.dataGView1.CurrentRow!=null)
            {
                ...
                this.textBox1.Text = this.dataGView1.Sum("采购金额").ToString();
                this.textBox2.Text = this.dataGView1.Sum("零售金额").ToString();
                reNum();
            }
        ...
        private bool checkNum(string wzId,string liuShuiId)
        {
            ...
            for (int i = 0; i < this.dataGView1.Rows.Count; i++)
            {
                if (this.dataGView1.Rows[i].Visible == true)
                {
                    ...
                    {
                        WJs.alert("从第" + (i + 1).ToString() + "行请输入物资");
                        return false;
                    }
            ...
            for (int i = 0; i < this.dataGView1.Rows.Count ; i++)
            {
                ...
                if (r != null)
                {
                    ...
                    if(this.dataGView1.Rows[i].Visible==true)
                    {
                        ...
                        xbNeiRong += r["GongHuoShangName"].Value.ToString() + "|";
        ...
        private void SaveData()
        {
            ...
            else
            {
                ...
                ac.Add("PLANID", Convert.ToInt32(outInfo["outID"]));
                ac.Add("PLANDATE", Convert.ToDateTime(this.textBox3.Text));//幅度段更改
        ...
        void ac5_ServiceLoad(object sender, YtClient.data.events.LoadEvent e)
        {
            ...
            //WJs.alert(e.Msg.Msg.Split('|')[0]);
            //if(e.Msg.Msg.Split('|').Length>1)
            //this.textBox3.Text = e.Msg.Msg.Split('|')[1];
        ...
        private void loadData()
        {
            ...
            this.textBox2.Text = outInfo["RuKuZongJinE"];
            this.textBox3.Text=outInfo["outDate"];
            ActionLoad ld = ActionLoad.Conn();
            DataTable tb = ld.Find("DbEditFindWareById", new object[] { this.selTextInpt2.Value.ToString() });
            if (tb != null && tb.Rows.Count > 0)
            {
                if (tb.Rows[0]["OUTORDER"].ToString() == "1")
                {
                    ChuKuOrder = "SlEditFindStockDetailOrderByP";
                }
                else if (tb.Rows[0]["OUTORDER"].ToString() == "2")
                {
                    ChuKuOrder = "SlEditFindStockDetailOrderByI";
                }
                else if (tb.Rows[0]["OUTORDER"].ToString() == "3")
                {
                    ChuKuOrder = "SlEditFindStockDetailOrderByG";
                }
                else if (tb.Rows[0]["OUTORDER"].ToString() == "4")
                {
                    ChuKuOrder = "SlEditFindStockDetailOrderByV";
                }
        ...
        private void loadXbData()
        {
            ActionLoad ld = ActionLoad.Conn();
            ld.Action = "Find";
            ld.Sql = "SlEditFindPlanDetailList";
        ...
        private void setDefalutStock(DataRow v,DataGridViewRow r)
        {
            ...
            else
            {
                    ...
                    r.Cells["BeiZhu"].Value = "";
                    r.Cells["GongHuoShangID"].Value = null;
                    r.Cells["GongHuoShangName"].Value =null;
        ...
        {
            ActionLoad ld = ActionLoad.Conn();
            DataTable tb = ld.Find("SlEditFindStockDetailById", new object[] { r.Cells["STOCKFLOWID"].Value });
            if (tb != null && tb.Rows.Count > 0)
            {
        ...
        private void ShenLingEdit_Load(object sender, EventArgs e)
        {
            this.WindowState = FormWindowState.Maximized;
            this.selTextInpt2.Enabled = false;//ye

            string setinput = "物资";
            this.dataGView1.addSql("DbEditFindWZ", setinput, "");
            this.dataGView1.addSql("SlEditFindStockDetail", "库存流水", "");
            ...
            this.selTextInpt2.TextChanged += new EventHandler(this.selTextInpt2_Leave); 
        ...
        {
            //if (this.selTextInpt1.Text.Trim().ToString() != this.selTextInpt1.textBox1.ToString())
            //{
            //    this.selTextInpt1.Text = null;
            //    this.selTextInpt1.Value = null;
            //    WJs.alert("申领库房填写有误，请重新输入！");
            ...
            if (this.selTextInpt1.Value != null && this.selTextInpt1.Value != "")
            {
                ...
                if (this.selTextInpt1.Value.ToString() != preValue1)
                {
                    if (preValue1 != null)
                    {
                        if (WJs.confirm("更改申领库房将会删除所有细表信息，确认更改申领库房？"))
                        {
                        ...
                        else
                        {
                            ...
                            this.selTextInpt1.Value = preValue1;
                            this.selTextInpt1.Text = preText1;
            ...
            //    }
                ...
                if(this.selTextInpt1.SelectedDataRow!=null)
                KuFangJiBie = "|" + this.selTextInpt1.SelectedDataRow["IFNDWARE"].ToString() + "|" + this.selTextInpt1.SelectedDataRow["IFRDWARE"].ToString()+"|0" ;
                ...
                //this.dataGView1.GetSql("物资").Ps = His.his.Choscode + "|" + this.selTextInpt2.Value + "|" + isAllowFKC.ToString() + "|{key}|{key}|{key}|{key}|{key}|{key}";
                if (this.selTextInpt1.Value != preValue1 || preValue1 == null)
                {
                    this.selTextInpt2.Enabled = true;//ye
                    this.selTextInpt2.Text = null;//ye
                    this.selTextInpt2.Value = null;//ye
                }
                preText1 = this.selTextInpt1.Text.ToString();
                preValue1 = this.selTextInpt1.Value.ToString();
            }

        }
        private void selTextInpt2_Leave(object sender, EventArgs e)
        {
            ...
            if (this.selTextInpt2.Value != null && this.selTextInpt2.Value != "")
            {
                if (this.selTextInpt2.Value.ToString() != preValue)
                {
                    if (preValue != null)
                    {
                        if (WJs.confirm("更改申领目的库房将会删除所有细表信息，确认更改申领目的库房？"))
                        {
                            //this.selTextInpt2.Value = null;
                            //this.selTextInpt2.Text = null;
                        ...
                        {
                            this.selTextInpt2.Value = preValue;
                            this.selTextInpt2.Text = preText;

                            return;
                        }
                    }
                    preText = this.selTextInpt2.Text.ToString();
                    preValue = this.selTextInpt2.Value.ToString();


                }
                if (this.selTextInpt2.SelectedDataRow != null)
                {
                    if (this.selTextInpt2.SelectedDataRow["OUTORDER"].ToString() == "1")
                    {
                        ChuKuOrder = "SlEditFindStockDetailOrderByP";
                    }
                    else if (this.selTextInpt2.SelectedDataRow["OUTORDER"].ToString() == "2")
                    {
                        ChuKuOrder = "SlEditFindStockDetailOrderByI";
                    }
                    else if (this.selTextInpt2.SelectedDataRow["OUTORDER"].ToString() == "3")
                    {
                        ChuKuOrder = "SlEditFindStockDetailOrderByG";
                    }
                    else if (this.selTextInpt2.SelectedDataRow["OUTORDER"].ToString() == "4")
                    {
                        ChuKuOrder = "SlEditFindStockDetailOrderByV";
                    }
                    //KuFangJiBie = "|" + this.selTextInpt1.SelectedDataRow["IFSTWARE"].ToString() + "|" + this.selTextInpt1.SelectedDataRow["IFNDWARE"].ToString() + "|" + this.selTextInpt1.SelectedDataRow["IFRDWARE"].ToString();
        ...
        private void updateMoney(int row)
        {
            ...
            if (r.Cells["ShuLiang"].Value != null && WJs.IsZs(r.Cells["ShuLiang"].Value.ToString()))
            {
                ...
                this.textBox1.Text = this.dataGView1.Sum("采购金额").ToString("f4");; ;
                this.textBox2.Text = this.dataGView1.Sum("零售金额").ToString("f4");
            }
            else
            {
        ...
        private void dataGView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == WzId.Index)  //更改物质
            {
                ActionLoad ld = ActionLoad.Conn();
                DataTable tb = null;
                tb = ld.Find(ChuKuOrder, new object[] { this.selTextInpt2.Value, this.dataGView1.Rows[e.RowIndex].Cells["WzId"].Value });
                if (tb != null && tb.Rows.Count > 0)
                {
            ...
            //{
                this.dataGView1.Rows[e.RowIndex].Cells["BZXS"].Value = hsxs;//this.dataGView1.Rows[e.RowIndex].Cells["BZXS"].Value = "1" + this.dataGView1.Rows[e.RowIndex].Cells["DanWei2"].Value.ToString() + "=" + this.dataGView1.Rows[e.RowIndex].Cells["ChangeRate"].Value.ToString() + this.dataGView1.Rows[e.RowIndex].Cells["DanWei1"].Value.ToString();
            }
        }
        ...
        private void dataGView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                this.dataGView1.GetSql("库存流水").Ps = this.selTextInpt2.Value + "|" + this.dataGView1.Rows[e.RowIndex].Cells["WzId"].Value;
            }
        ...
        private void toolStripButton5_Click(object sender, EventArgs e)
        {
            ...
            {
                ActionLoad ld = ActionLoad.Conn();
                DataTable tb = ld.Find("PlanMainDaYinQuery", new object[] { outInfo["outID"] });
                ...
                if (tb != null && tb.Rows.Count > 0)
                {
                    ...
                    pp.Add("BiaoTi", tb.Rows[0][0].ToString() + "  【" + outInfo["ChuKuKuFangName"]   + "】  " + "申领单");
                    pp.Add("FangXiang", "从  【" + outInfo["MuDiKuFangName"] + "】  申领到  【" + outInfo["ChuKuKuFangName"] + "】");
                    pp.Add("Time", "时间：" + tb.Rows[0][5].ToString());
                    pp.Add("LeiBie", "");
                    pp.Add("BianHao", "类别：申领单");
                    ...
                    app.LoadPlug("RepEdit.RepView", new object[] { "CLWZSLDYCS001", pp, false }, false);
                }
            }
            else
            {
