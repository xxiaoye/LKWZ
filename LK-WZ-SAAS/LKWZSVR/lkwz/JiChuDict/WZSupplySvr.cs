        ...
        public object Run(YiTian.db.Dao dao, YtService.data.OptData data, out string msg)
        {
            ...
            if ("SaveChangShangInfo".Equals(ac))
            {
                ...
                if (data.Param["SUPPLYID"] == null)//新增
                {
                    ...
                    Opt findwzsupplyidInfo = OptContent.get("findwzsupplyididInfo");
                    ObjItem objj = DaoTool.ExecuteScalar(dao, findwzsupplyidInfo, pa);

                    long a = DaoTool.Seq(dao, "LKWZ.SEQWZSUPPLY");

                    for (long i = a; i <= objj.ToLong(); i++)
                    {
                        a = DaoTool.Seq(dao, "LKWZ.SEQWZSUPPLY");

                    }
                    pa["SUPPLYID"] = a;


                    //pa["SUPPLYID"] = DaoTool.Seq(dao, "LKWZ.SEQWZSUPPLY");





                   Opt saveInfo = OptContent.get("SaveChangShangInfo");
                  Opt  name = OptContent.get("SaveChangShangInfo_Name");
                ...
                else   //修改
                {
                   ...
                   Opt   updataInfo = OptContent.get("UpdataChangShangInfo");
                   Opt  updataname = OptContent.get("UpdataChangShangInfo_Name");
             ...
             if ("DelChangShangInfo".Equals(ac))
            {
                Opt Del = OptContent.get("DelChangShangInfo");
                ...
                Opt values1 = OptContent.get("DelChangShangInfo_Use1");
                Opt values2 = OptContent.get("DelChangShangInfo_Use2");
                Opt values3 = OptContent.get("DelChangShangInfo_Use3");
                Opt values4 = OptContent.get("DelChangShangInfo_Use4");
                ...
                if ( !Obj1.IsNull  || !Obj2.IsNull || !Obj3.IsNull || !Obj4.IsNull)
                {
                    msg = "不能删除已使用的厂商，只能停用！";
                    return "ok";
                }
             ...
             {
                 Opt stop = OptContent.get("StartOrStopChangShangInfo");
             ...
             {
                 Opt start = OptContent.get("StartOrStopChangShangInfo");
